...
export function FinanceDashboard() {
  ...
  return (
    <div className="space-y-6 pb-20">
      {/* Title on its own row on a phone, buttons wrap onto their own row and
          share the width equally so the label text doesn't have to break. */}
      <div className="flex flex-col gap-3 sm:flex-row sm:justify-between sm:items-center">
        <h1 className="text-2xl font-semibold text-gray-900">Finance Dashboard</h1>
        <div className="flex gap-2 sm:gap-3">
          <button
            onClick={() => setIsExpenseModalOpen(true)}
            className="bg-red-50 text-red-700 border border-red-200 px-3 sm:px-4 py-2 rounded-md hover:bg-red-100 flex items-center justify-center font-medium flex-1 sm:flex-none whitespace-nowrap"
          >
            <ArrowDownRight className="w-5 h-5 mr-2" /> Add Expense
          </button>
          <button
            onClick={() => setIsPaymentModalOpen(true)}
            className="bg-green-600 text-white px-3 sm:px-4 py-2 rounded-md hover:bg-green-700 flex items-center justify-center font-medium flex-1 sm:flex-none whitespace-nowrap"
          >
